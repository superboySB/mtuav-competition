    ...
    for(int i = 0; i < num_drone; i++){
        ...
        for(int j = 0; j < num_drone; j++){
            if(i!=j){
                ...
                if(val < 1.0){
                    if(VERBOSE == 3)
                        LOG(WARNING) << "Collision between agent " << i << " and agent " << j << " at MPC step " << prob_data[i].mpc_step;   
        ...
        for(int m = 0; m < num_obs; m++){
            ...
            if(val < 1.0){
                if(VERBOSE == 3)
                    LOG(WARNING) << "Collision between agent " << i << " and obstacle " << m << " at MPC step " << prob_data[i].mpc_step;   
...
void Simulator :: runAlgorithm(){
    ...
    if(VERBOSE == 4)
        LOG(INFO) << "Time to compute = " << total_time.count()/1000.0 << " s, Planning Frequency = " << 1000.0/total_time.count();
}
...
void Simulator :: checkViolation(){
    ...
    for(int i = 0; i < num_drone; i++){
        ...
        if(std::floor(prob_data[i].x_init/prob_data[i].thresold)*prob_data[i].thresold > prob_data[i].x_max || std::ceil(prob_data[i].x_init/prob_data[i].thresold)*prob_data[i].thresold < prob_data[i].x_min){
			out_space = true;
            LOG(WARNING) << "Positional bounds not satisfied in x " << prob_data[i].x_init << " " << prob_data[i].x_min << " " << prob_data[i].x_max;
        }
		if(std::floor(prob_data[i].y_init/prob_data[i].thresold)*prob_data[i].thresold > prob_data[i].y_max  || std::ceil(prob_data[i].y_init/prob_data[i].thresold)*prob_data[i].thresold < prob_data[i].y_min){
			out_space = true;
            LOG(WARNING) << "Positional bounds not satisfied in y " << prob_data[i].y_init << " " << prob_data[i].y_min << " " << prob_data[i].y_max;
        }
...
void Simulator :: runSimulation(){
    ...
    for(sim_iter = 0; sim_iter < max_time/dt; sim_iter++){
        ...
        if(VERBOSE == 2){
            LOG(INFO) << "Simulation time = " << sim_iter;
            LOG(INFO) << "Distance to goals = " << dist_to_goal.transpose();
        }
...
void Simulator :: saveMetrics(){
    ...
    if(success && !collision_agent && !collision_obstacle){
        ...
        LOG(INFO) << "________ SUCCESS! _________";
        LOG(INFO) <<"Mission completion time = " << mission_time << " s";
        LOG(INFO) <<"Total time to compute = " << total_time.count()/1000.0 << " s";
        LOG(INFO) <<"Average time to compute = " << total_time.count()/1000.0/prob_data[0].mpc_step << " s";
        LOG(INFO) <<"Average run time per agent = " << total_time.count()/1000.0/prob_data[0].mpc_step/num_drone << " s";
        LOG(INFO) <<"Average smoothness = " << avg_smoothness << " ms^-2";
        LOG(INFO) <<"Average trajectory length = " << avg_traj_length << " m";
        LOG(INFO) <<"Average inter-agent dist = " << avg_inter_agent_dist << " m";
        LOG(INFO) <<"Smallest inter-agent dist = " << min_inter_agent_dist << " m";
        LOG(INFO) <<"Average obs-agent dist = " << avg_agent_obs_dist << " m";
        LOG(INFO) <<"Smallest obs-agent dist = " << min_agent_obs_dist << " m";
    }
    else{
        success = false;
        LOG(INFO) << "________ FAILURE! _________";
        if(collision_agent)
            LOG(WARNING) << "Inter-agent collision";
        else if(collision_obstacle)
            LOG(WARNING) << "Obstacle-agent collision";
        else
            LOG(WARNING) << "Goal not Reached";
    }
