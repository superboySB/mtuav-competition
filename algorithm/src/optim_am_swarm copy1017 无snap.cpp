...
namespace Optim
{
	...
	void initializeOptimizer(probData &prob_data, int VERBOSE){
		...
		prob_data.dist_stop = params["dist_stop"].get<float>();  // 到达目的地
		...
		prob_data.prox_agent = params["prox_agent"].get<float>();  // 与飞机间安全距离
		prob_data.prox_obs = params["prox_obs"].get<float>();   // 与障碍间安全距离
		prob_data.buffer = params["buffer"].get<float>();  // 最小安全距离
