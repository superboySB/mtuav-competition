...
namespace Optim
{
	...
	void checkResiduals(probData &prob_data, int VERBOSE)
	{
		...
		if(prob_data.res_x_static_obs_norm > thresold || prob_data.res_y_static_obs_norm > thresold){
			LOG(WARNING) << "Drone " << prob_data.id_badge << " MPC Step " << prob_data.mpc_step;
			LOG(WARNING) << "Static Obstacle Constraints = " << prob_data.res_x_static_obs_norm << " " << prob_data.res_y_static_obs_norm;
		}
		if(prob_data.res_x_drone_norm > thresold || prob_data.res_y_drone_norm > thresold || prob_data.res_z_drone_norm > thresold){
			LOG(WARNING) << "Drone " << prob_data.id_badge << " MPC Step " << prob_data.mpc_step;
			LOG(WARNING) << "Inter-Agent Collision Constraints = " << prob_data.res_x_drone_norm << " " << prob_data.res_y_drone_norm << " " << prob_data.res_z_drone_norm;
		}
		if(prob_data.res_x_vel_norm > thresold || prob_data.res_y_vel_norm > thresold || prob_data.res_z_vel_norm > thresold){
			LOG(WARNING) << "Drone " << prob_data.id_badge << " MPC Step " << prob_data.mpc_step;
			LOG(WARNING) << "Velocity Constraints = " << prob_data.res_x_vel_norm << " " << prob_data.res_y_vel_norm << " " << prob_data.res_z_vel_norm;
		}
		if(prob_data.res_x_acc_norm > thresold || prob_data.res_y_acc_norm > thresold || prob_data.res_z_acc_norm > thresold){
			LOG(WARNING) << "Drone " << prob_data.id_badge << " MPC Step " << prob_data.mpc_step;
			LOG(WARNING) << "Acceleration Constraints = " << prob_data.res_x_acc_norm << " " << prob_data.res_y_acc_norm << " " << prob_data.res_z_acc_norm;
		}
		if(prob_data.res_x_jerk_norm > thresold || prob_data.res_y_jerk_norm > thresold || prob_data.res_z_jerk_norm > thresold){
			LOG(WARNING) << "Drone " << prob_data.id_badge << " MPC Step " << prob_data.mpc_step;
			LOG(WARNING) << "Jerk Constraints = " << prob_data.res_x_jerk_norm << " " << prob_data.res_y_jerk_norm << " " << prob_data.res_z_jerk_norm;
		}
		if(prob_data.res_x_snap_norm > thresold || prob_data.res_y_snap_norm > thresold || prob_data.res_z_snap_norm > thresold){
			LOG(WARNING) << "Drone " << prob_data.id_badge << " MPC Step " << prob_data.mpc_step;
			LOG(WARNING) << "Snap Constraints = " << prob_data.res_x_snap_norm << " " << prob_data.res_y_snap_norm << " " << prob_data.res_z_snap_norm;
		}
		if(prob_data.res_x_ineq_norm > thresold || prob_data.res_y_ineq_norm > thresold || prob_data.res_z_ineq_norm > thresold){
			LOG(WARNING) << "Drone " << prob_data.id_badge << " MPC Step " << prob_data.mpc_step;
			LOG(WARNING) << "Workspace Constraints = " << prob_data.res_x_ineq_norm << " " << prob_data.res_y_ineq_norm << " " << prob_data.res_z_ineq_norm;
		}
	}
	...
	void initializeOptimizer(probData &prob_data, int VERBOSE){
		...
		if(prob_data.num_drone!=0){
			...
			LOG(WARNING) << "Only one drone in environment";
		}
	...
	void deployAgent(probData &prob_data, int VERBOSE){
		...
		if(prob_data.mpc_step == 0){
			LOG(INFO) <<"Initializing Drone " << prob_data.id_badge;
		...
		if(prob_data.mpc_step > 0){
			...
			// 如果存在其他无人机，该函数计算与每个无人机的相对位置，
			// 并基于这些信息计算与这些无人机相关的角度 alpha_drone 和 beta_drone。
		...
		// @ Apply control input (假设我们获取到的都是groundtruth，没有噪声)				
